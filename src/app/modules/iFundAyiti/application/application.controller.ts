...
const createApplication = catchAsync(async (req: Request, res: Response) => {
  ...
  const supporting_documents = getMultipleFilesPath(
    req.files,
  ...
  if (proof_of_address)
    documentTypes.push({ type: 'proof_of_address', url: proof_of_address });
  if (business_plan)
    documentTypes.push({ type: 'business_plan', url: business_plan });

  if (supporting_documents && Array.isArray(supporting_documents)) {
    supporting_documents.forEach((url: string) => {
      documentTypes.push({
...
const updateApplicationStatus = catchAsync(
  async (req: Request, res: Response) => {
    const result = await ApplicationServices.updateApplicationStatusToDB(
      req.params.id,
      req.body,
      req.user,
    );
...
const getMonthlyChart = catchAsync(async (req: Request, res: Response) => {
  ...
  const result =
    await ApplicationServices.getMonthlyApplicationChartFromDB(year);
  return sendResponse(res, {
    success: true,
    statusCode: StatusCodes.OK,
...
const getDonationAmountChart = catchAsync(
  async (req: Request, res: Response) => {
    const year = req.query.year as string;
...
const getApplicationStatusStats = catchAsync(
  async (req: Request, res: Response) => {
...
const getRecentApplications = catchAsync(
  async (req: Request, res: Response) => {
...
const winnerSelection = catchAsync(async (req: Request, res: Response) => {
  const result = await ApplicationServices.winnerSelection(
    req.params.id,
    req.body,
    req.user,
  );
